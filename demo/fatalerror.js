...
function my_listener(request, response) {
  switch (count++) {
    case 0:
      ...
      response.write('\nRunning NodeReport fatal error demo... refresh' +
        'page to trigger excessive memory usage (application will terminate)');
      response.end();
      break;
    case 1:
      console.log('heap_oom.js: allocating excessive ' +
...
console.log('fatalerror.js: Note: heap default is 1.4Gb, ' +
  'use --max-old-space-size=<size in Mb> to change');
console.log('fatalerror.js: Go to http://<machine>:8080/ ' +
